...
class AndroidMainActivity : AppCompatActivity(),
                    ...
                    object : TelephonyCallback(), TelephonyCallback.CallStateListener {
                        override fun onCallStateChanged(state: Int) {
                            if (appState.pauseOnCall && TelephonyManager.CALL_STATE_OFFHOOK == state) {
                                if (appState.currentTimerRun?.timerState == com.github.rs17.annoyingpomodoro_lib.TimerRun.TimerState.running) {
    ...
    private fun getPrefs(prefs: SharedPreferences, updatedKey: String){
        ...
        if(appState.pauseOnCall){
            ...
            readCallStatePermissionLauncher.launch(READ_PHONE_STATE)
        }
